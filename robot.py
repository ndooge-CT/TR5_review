...
if len(sys.argv) > 1:
    ...
    from world.text import world
# select maze from maze folder
# default to obstacles module 
if len(sys.argv) > 2:
    obstacles = import_helper.dynamic_import('maze.maze_mod.{}'.format(sys.argv[2]))
else:
...
def do_mazerun(robot_name,edge):
    ...
    edge_case(robot_name,edge)

    return True, f"{robot_name}: I am at the {edge} edge."

def compile_moves(path,edge):
    curr = path[0]
    count = 0
    ...
    if len(moves)>1:
        if edge == 'top' or edge == 'bottom':
            if moves[:-1][0] == 'W' or moves[:-1][0] == 'E':
                moves[:-1][1] = 1
        elif edge == 'left' or edge == 'right':
            if moves[:-1][0] == 'S' or moves[:-1][0] == 'N':
                moves[:-1][1] = 1
        return moves

def compile_commands(compiled_moves):
    commands = []
    direction = world.current_direction_index
    for move in compiled_moves:
        turn, direction = get_next_direction(move[0],direction)
        commands += turn
        commands.append(f"forward {move[1]}")
    return commands

def get_next_direction(direction,current_direction):
    direction = ["N","E","S","W"].index(direction)
    if direction == current_direction:
        return [""],direction
    elif direction == (current_direction+1)%4:
        return ["right"],direction
    elif direction == (current_direction-1)%4:
        return ["left"],direction
    elif direction == (current_direction+2)%4:
        return ["right","right"],direction

def edge_case(robot_name,edge):
    direction = world.current_direction_index
    edge_case = ['top','right','bottom','left'].index(edge)

    if edge_case != direction:
        edge_case = ["N","E","S","W"][edge_case]
        turn,direction = get_next_direction(edge_case,direction)
